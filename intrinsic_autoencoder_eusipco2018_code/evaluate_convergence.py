...
def plot_loss( loss_name, history_name, datasets, window_size=1 ):
  ...
  print( 'average ' + loss_name + ' on ' + history_name + ( ': %g' % np.mean( data ) ))
...
datasets_b.add( 'lf_patch_autoencoder' )


# switch between different dataset groups for visualization
# can be empty, then all are averaged
datasets = set()
#datasets.add( 'lf_patch_lytro' )
#datasets.add( 'lf_patch_stanford' )
#datasets.add( 'lf_patch_hci' )
#datasets = datasets_i
